...
def model_1(data: List[TrialData], inference_data_path: Path, file_name: Path, persistence: Persistence) -> None:
    ...
    unique_participants = {p.participant for p in data}
    ...
    df = pd.DataFrame(
        {
            ...
            "goalkeeper_displacement": [p.goalkeeper_displacement for p in filtered_data],
    ...
    priors = {
        "goalkeeper_displacement": flat_prior,
        "advertisement_direction": flat_prior,
        "goalkeeper_displacement:advertisement_direction": flat_prior,
    }
